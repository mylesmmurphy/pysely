...
test("shows the generated module on demand", async ({ page }) => {
  await page.goto("/playground/");
  await expect(page.locator("#playground-status")).toHaveText("Compiled", { timeout: 40_000 });
  const details = page.locator("#playground-generated");
  await expect(details).not.toHaveAttribute("open", "");
  await details.locator("summary").click();
  await expect(details).toHaveAttribute("open", "");
  const generated = () => page.evaluate(() => {
    const monaco = (window as any).monaco;
    return monaco.editor.getModel(monaco.Uri.parse("file:///workspace/database.py")).getValue();
  });
  await expect.poll(generated).toContain("Generated by `pysely codegen`");
  await expect.poll(generated).toContain("class DatabaseSchema(GeneratedSchema[DatabaseClient]):");
  ...
  // ...but they are still syntax highlighted.
  expect(await page.locator("#playground-generated-code .mtk1, #playground-generated-code [class^=mtk]").count()).toBeGreaterThan(10);
